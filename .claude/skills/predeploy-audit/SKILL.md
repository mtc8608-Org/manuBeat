...
2. **Launch both agents in one message** (parallel, background):
   ...
   - **Auth surface** — `exposure-auditor` subagent. Prompt it to gate the full
     ...
     invariant. Tell it to cover **every domain present in this repo**, not just
     the framework. Ask for findings marked **BLOCKER / SHOULD-FIX / OK-noted**
     with `file:line`.
...
- The TODO file is **gitignored in manuSpine** (a public framework repo should
  not publish a live list of its own unfixed holes). Forks may commit their copy
  — that is a per-repo call, not a framework rule.
- In manuSpine, a finding in framework code is by definition inherited by every
  fork. Fixing it here is the fix for all of them; do not let a fork patch it
  locally (CLAUDE.md "Framework downstream").
...
- Re-run after every `pull-upstream` merge (in a fork) or `port-upstream` (here)
  — a framework change can silently move an op out of its tier or reopen a
  scoping hole.

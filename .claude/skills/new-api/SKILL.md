...
3. **Clone the closest existing module**, don't write from scratch:
   ...
   - REST → `routes/framework/files.js` (files) or `routes/framework/compute.js` (Python orchestration). Register in `backend.js` under the domain banner.
...
6. **Keep it domain-free.** manuSpine is the framework: nothing added here may be domain-specific — domain capabilities belong in a fork. Changes made here reach forks when they run `git merge upstream/master`.

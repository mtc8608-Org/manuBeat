...
(the third-party React adapter) was removed and replaced by the owned shell
component `pwa/src/components/shell/EChart.tsx`; the echarts **engine**
...
charts). Registered in `code-reuse.md`'s decision guide, CLAUDE.md's shell
list, and the seeded dev-guide shell overview.
...
`components/shell/EChart` (prop `option` is the same; `ReactECharts`'
